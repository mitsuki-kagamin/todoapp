...
fn finish(cont: Cont, result: TaskResult, cache: &Cache, q: &mut CacheQ) -> Bytes {
    match cont {
        ...
        Cont::Patch(id) => {
            // Before the response is staged, never after: a mirror on any worker must not be
            // able to answer with the pre-patch body once this reply is on the wire.
            CACHE_VERSION.fetch_add(1, Ordering::Release);
            match result {
            TaskResult::Todo { send, cache: fill } => {
...
/// Longest request the mirror will remember. k6 and wrk both send well under this.
const SPEC_MAX: usize = 256;

/// Bumped whenever a write changes what a cached response should say, **before** the mutating
/// response is staged. A speculative hit reads it and compares; the mirror is not allowed to
/// answer across a change it did not see.
///
/// On x86 this is a read-mostly line that stays Shared in every core's L1 for the whole
/// benchmark, so the load costs nothing and generates no coherence traffic.
static CACHE_VERSION: AtomicU64 = AtomicU64::new(0);

/// The last request this worker answered from cache, and the answer it gave.
///
/// The point is not to parse faster -- parsing measured 0.09 us. It is to skip the whole path:
/// no request-line compare, no header scan for Content-Length, no id extraction, no `ArcSwap`
/// load with its hazard-pointer debt slot. One version load and one memcmp instead.
struct Spec {
    req: [u8; SPEC_MAX],
    req_len: u16,
    resp: Bytes,
...
///
/// Matching on a prefix, or on `n >= req_len`, would mean a pipelined
/// `GET a\r\n\r\nGET b\r\n\r\n` gets one response and the second request is silently lost --
/// a desync that looks like a client bug. Requiring the buffer to have been empty and the
/// length to match exactly makes a false positive impossible by construction.
#[inline(always)]
fn spec_hit(spec: &Option<Spec>, bytes: &[u8]) -> Option<Bytes> {
    let sp = spec.as_ref()?;
    if sp.req_len as usize != bytes.len() {
        return None;
    }
    if CACHE_VERSION.load(Ordering::Acquire) != sp.version {
        return None;
    }
    (sp.req[..bytes.len()] == *bytes).then(|| sp.resp.clone())
}
...
fn pump(
    ...
    loop {
        ...
        let body = c.buf[head_len..total].to_vec();
        // Read before the lookup, not after. If a write lands in between, the version stored
        // is the older one, so the next speculative compare fails and takes the full path --
        // stale in the safe direction.
        let version = CACHE_VERSION.load(Ordering::Acquire);
        let decision = plan(&head, &body, cache, q);

        match decision {
            Decision::Respond(response) => {
                // Arm only from a GET answered 200 out of cache that was the entire buffer:
                // the next read's compare is whole-buffer, so anything else would let a
                // pipelined second request be swallowed.
                if c.out_len == 0
                    && total == c.buf.len()
                    && total <= SPEC_MAX
                    && matches!(head, Head::Get { .. })
                    && response.starts_with(b"HTTP/1.1 200 OK")
                {
                    let mut req = [0u8; SPEC_MAX];
                    req[..total].copy_from_slice(&c.buf[..total]);
                    *spec = Some(Spec {
                        req,
                        req_len: total as u16,
                        resp: response.clone(),
                        version,
                    });
                }
...
fn worker(cfg: WorkerCfg) -> std::io::Result<()> {
    ...
    // Worker-local, so reading it costs nothing and no other thread can see it.
    let mut spec: Option<Spec> = None;
    // A/B switch: with it off the mirror is never consulted and every request is parsed.
    let use_spec = !std::env::var("TODOAPP_SPEC").map(|v| v == "0").unwrap_or(false);
        ...
        while let Some(cqe) = cq.next() {
            ...
            if tag == TAG_RECV {
                if res > 0 {
                    let c = &mut conns[fd];
                    let len = c.buf.len();
                    // Speculation is only tried on a read that is a whole request by itself.
                    let fresh = len == 0 && c.out_len == 0 && !c.awaiting;
                    let n = res as usize;
                    // The kernel wrote into the spare capacity; publish it.
                    unsafe { c.buf.set_len(len + n) };
                    let hit = if fresh && use_spec {
                        spec_hit(&spec, &c.buf[..n])
                    } else {
                        None
                    };
